...
class Intersection:
    def __init__(
        self, nbpoints, points, params, crv: Geom2d_Curve, interval, valid: bool = True, two_point_on_same_line=False
    ) -> None:
        self._crv = crv
...
class Rectangle:
    ...
    def split(self, split_point=None, auto_ajust=True, check_area=True, tol=9e-3, discarded=None):
        ...
        if split_point is None:
            ...
            if auto_ajust:
                ...
                if (
                    split_point[0] is None
                    or math.fabs(split_point[0] - min_x) < tol
                    or math.fabs(split_point[0] - max_x) < tol
                ):
                    split_point[0] = (min_x + max_x) / 2
                if (
                    split_point[1] is None
                    or math.fabs(split_point[1] - min_y) < tol
                    or math.fabs(split_point[1] - max_y) < tol
                ):
            ...
            self.sub_rects[0].points = [
                self.points[0],
                (split_point[0], self.points[0][1]),
                (self.points[0][0], split_point[1]),
                split_point,
            ]
            self.sub_rects[1].points = [
                (split_point[0], self.points[1][1]),
                self.points[1],
                split_point,
                (self.points[1][0], split_point[1]),
            ]
            self.sub_rects[2].points = [
                (self.points[2][0], split_point[1]),
                split_point,
                self.points[2],
                (split_point[0], self.points[2][1]),
            ]
            self.sub_rects[3].points = [
                split_point,
                (self.points[3][0], split_point[1]),
                (split_point[0], self.points[3][1]),
                self.points[3],
            ]
    ...
    def contains(self, point: List[float], boundary=True, tol=1e-4) -> bool:
        ...
        if boundary:
            return (
                point[0] - min_x >= -tol
                and max_x - point[0] >= -tol
                and point[1] - min_y >= -tol
                and max_y - point[1] >= -tol
            )
    ...
    def onBoundary(self, point: List[float], tol=1e-4):
        min_x, max_x, min_y, max_y = self.getBoundary()
        return (
            math.fabs(point[0] - min_x) < tol
            or math.fabs(point[0] - max_x) < tol
            or math.fabs(point[1] - min_y) < tol
            or math.fabs(point[1] - max_y) < tol
        )
...
class Intersector:
    ...
    def intersectWithLine(
        self, line: Geom2d_Line, rect: Rectangle, curve: Geom2d_Curve, interval, tol=1e-4
    ) -> Intersection:
...
def curveInRect(crv: Geom2d_Curve, interval, rect: Rectangle, intersection: Intersection, tol=9e-3):
    if intersection.NbPoints > 1:
        return False
    umin, umax = interval
    return rect.contains(geom_utils.gp_to_numpy(crv.Value(umin)), boundary=False, tol=tol) or rect.contains(
        geom_utils.gp_to_numpy(crv.Value(umax)), boundary=False, tol=tol
    )
                ...
                if status[(idx + 1) % 4] > 0:
                    if item != status[(idx + 1) % 4]:
                        ...
                        if vis == 1 or vis == 3:
                            ...
                            rectangle.sub_rects[end_index].leaf_info = make_triangles(
                                face=face,
                            ...
                            rectangle.sub_rects[end_index].leaf_info = make_triangles(
                                ...
                                interval=intersection.Parameters[::-1] if reverse else intersection.Parameters,
                                end_points=(
                                    intersection.Points[reverse],
                                    rectangle.points[end_index],
                                    intersection.Points[not reverse],
                                ),
                            )
                ...
                if not corner_point and status[(idx + 2) % 4] > 0 and item != status[(idx + 2) % 4]:
                    ...
                    if vis == 1 or vis == 3:
                        ...
                        if np.linalg.norm(point1 - end_point) - np.linalg.norm(point2 - other_point) > tol:
                            ...
                            rectangle.split(oppo_point, auto_ajust=False, tol=tol)
                            rectangle.is_leaf = False
                            rectangle.sub_rects[clockwise_index[(idx + 3) % 4]].leaf_info = make_triangles(
                                face=face,
                                curve=intersection.Curve[0],
                                interval=intersection.Parameters[::-1] if reverse else intersection.Parameters,
                            ...
                            rectangle.split(oppo_point, auto_ajust=False, tol=tol)
                            rectangle.is_leaf = False
                            rectangle.sub_rects[clockwise_index[idx]].leaf_info = make_triangles(
                                face=face,
                                curve=intersection.Curve[0],
                                interval=intersection.Parameters[::-1] if reverse else intersection.Parameters,
                                end_points=(point1, oppo_point, point2),
                            ...
                            rectangle.split(oppo_point, auto_ajust=False, tol=tol)
                            rectangle.is_leaf = False
                            rectangle.sub_rects[end_index].leaf_info = make_boundary_rect(
                                face=face,
                                curve=intersection.Curve[0],
                                interval=intersection.Parameters[::-1] if reverse else intersection.Parameters,
...
def pointOnLine(p1, p2, point, tol=9e-3):
    return (
        (np.linalg.norm(p2 - point) < tol or np.linalg.norm(p1 - point) < tol)
        or np.dot(p2 - p1, point - p1) >= 0
        and distance_point_to_line(p1, p2, point) < tol
    )
...
def splitRectangle(face: Face, rectangle, curves, max_split=7, tol=0.7, distance_tol=1e-4, split_all=False):
    ...
    while len(stack) > 0:
        ...
        for curve, interval in curves:
            ...
            if (
                curveInRect(curve, interval, rect, intersection, tol=distance_tol)
                or intersection.NbPoints > 2
                or intersection.TwoPointOnSameLine
                or not chordErrorCheckInRect(intersection, rect, tol=tol)
            ):
                split_flag = True
                break
        ...
        if len(intersections) > 1:
            ...
            if len(intersections) == 2:
                ...
                if (
                    point is not None
                    and rect.contains(point, tol=distance_tol)
                    and not rect.isCorner(point, tol=distance_tol)
                ):
                    split_point = point
        ...
        if split_flag or split_all:
            ...
            if len(intersections) == 0 and (
                (not rect.anyCornersInFace(face)) and face.visibility_status(rect.center()) == 1
            ):
                rect.discarded = True
                continue
            rect.leaf_info = intersections
